...
int main( int argc, char* argv[] )
{
    ...
    char *numbits = argv[1];
    int  repetitions = atoi(numbits);
    char *avx_yes = argv[2];
    printf("Sum %d repetitions, option %c \n", repetitions, *avx_yes);
    ...
    /*
     * repetitions: the more the better for avx, this depends the main optimized in gather_load. Full block gather
     * blocklen:
     * number of elems in gap:1
     */
    if(*avx_yes=='s') {
        if( run_tests & DO_CONSTANT_GAP ) {
            printf( "\noptimized constant indexed gap\n\n" );
            ddt = create_optimized_indexed_constant_gap_ddt( repetitions, 2, 1 );
            MPI_DDT_DUMP( ddt );
