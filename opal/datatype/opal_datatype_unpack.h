...
/* avx sactter unpack */
static inline void
unpack_predefined_data_avx( opal_convertor_t* CONVERTOR,
                        const dt_elem_desc_t* ELEM,
                        size_t* COUNT,
                        unsigned char** packed,
                        unsigned char** memory,
                        size_t* SPACE )
{
    ...
    if( 0 != do_now ) {
        ...
        /* each vector can deal with multi blocks */
            ...
            int blocks_in_VL = vector_length/do_now_bytes/4;
            ...
            __mmask16 load_mask;
            __m512i temp_src;
            __m512i xt = _mm512_loadu_si512((__m512*)off_sets);
            ...
            {
                __m512i vsrc = _mm512_mask_load_epi32 (temp_src, load_mask, *(packed));
                _mm512_mask_i32scatter_epi32 (_memory, load_mask, xt, vsrc, 4);
                /*
...
#define UNPACK_PREDEFINED_DATATYPE_AVX( CONVERTOR,    /* the convertor */    \
                                    ELEM,         /* the basic element to be packed */ \
                                    COUNT,        /* the number of elements */ \
                                    PACKED,       /* the destination pointer (char*) */ \
                                    MEMORY,       /* the source pointer (char*) */ \
                                    SPACE )       /* the space in the destination buffer */ \
unpack_predefined_data_avx( (CONVERTOR), (ELEM), &(COUNT), &(PACKED), &(MEMORY), &(SPACE) )
