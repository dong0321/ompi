...
/** @file
 *
 * This is the "sve" component source code.  It contains the
...
static int sve_component_open(void)
{
    opal_output(ompi_op_base_framework.framework_output, "sve component open");
...
static int sve_component_close(void)
{
    opal_output(ompi_op_base_framework.framework_output, "sve component close");
...
static char *sve_component_version;
...
static int sve_component_register(void)
{
    ...
    mca_op_sve_component.hardware_available = true;
...
static int sve_component_init_query(bool enable_progress_threads,
                                        bool enable_mpi_thread_multiple)
{
    opal_output(ompi_op_base_framework.framework_output, "sve component init query");
    ...
    */
    ...
    /* Note that we used MCA parameters to fill in the
       ...
       are only used in this sve to allow simulating different
       types of hardware). */
