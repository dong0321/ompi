                 ...
                 opal_buffer_t *buf)
{
    ...
    for(i=1; i <= nprocs/2; i*=2) for(d=1; d >= -1; d-=2) {
     ...
     redo:
        ...
        OBJ_RETAIN(buf);
        if(idx!=0){
            printf("\n dong bmg rbcast from %d to %d ", ORTE_PROC_MY_NAME->vpid, idx);
            ...
            if (ORTE_SUCCESS == (ret = opal_dss.unpack(data, &command, &cnt, ORTE_DAEMON_CMD))) {
                 ...
                 * handle connection losses from our downstream peers */
                 if (ORTE_DAEMON_EXIT_CMD == command ||
                      ...
                      * knows what to do
                      */
                     OPAL_OUTPUT_VERBOSE((5, orte_grpcomm_base_framework.framework_output, "%s grpcomm:bmg:xcast updating daemon nidmap",                                                                                                                                        ORTE_NAME_PRINT(ORTE_PROC_MY_NAME)));
